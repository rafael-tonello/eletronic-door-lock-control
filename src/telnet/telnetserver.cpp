...
void TelnetServer::picUpkNextClient()
{
    ...
    if (client) {
        ...
        clients[id] = std::make_shared<CliInfoExtented>();
...
Promise<TpNothing>::smp_t TelnetServer::TalkToAClient(std::shared_ptr<CliInfoExtented>cli)
{
...
void TelnetServer::processLineReceivedFromClient(std::shared_ptr<CliInfoExtented>cli, String receivedLine)
{
    ...
    scheduler.run([=](){
        ...
        if (cli->authenticationState != CliInfo::AS_AUTHENTICATED)
        {
            ...
            this->validadtePassword(line)->then([=](bool authenticatedWithSucess){
...
void TelnetServer::sendToClient(std::shared_ptr<CliInfoExtented>cli, String message, bool breakLine)
{
    if (clients.count(cli->id) == 0)
    {
        nLog.warning("Trying to send data to a client that is not in the 'clients' map");
        return;
    }
...
Promise<TpNothing>::smp_t TelnetServer::sendInitialInfoToClient(std::shared_ptr<CliInfoExtented>cli)
{
...
void TelnetServer::finalizeClient(std::shared_ptr<CliInfoExtented>cli)
{
    cli->theClient.stop();
...
void TelnetServer::setClientEcho(std::shared_ptr<CliInfoExtented>cli, bool enabled)
{
    if (!cli->theClient.connected())
        return;
